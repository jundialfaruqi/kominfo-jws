    ...
    $(document).ready(function() {
        ...
        function loadAudioCacheFromLocalStorage(currentSlug) {
            try {
                ...
                if (!cacheData || !cacheTimestamp || !cacheSlug) {
                    console.log(
                        'Tidak ada cache audio di localStorage');
                    return null;
                }
                ...
                error: function(xhr, status, error) {
                    ...
                    if (xhr.status === 404 || xhr.status ===
                        ...
                        clearAudioCacheFromLocalStorage(
                            slug);
                        console.log(
                            'Cache localStorage dibersihkan'
                        );
                    }
                    ...
                    if (cachedAudioUrls.length > 0) {
                        ...
                        console.log(
                            'Mencoba mengambil audio lagi dalam 5 menit...'
                        );
            ...
            try {
                ...
                if (month !== parseInt(currentMonthValue) ||
                    ...
                    try {
                        ...
                        console.log(
                            "Data bulan baru berhasil diambil, memperbarui input hidden..."
                        );
        ...
        function checkDayChange() {
            ...
            if (currentDate !== storedDate) {
                ...
                console.log(
                    'Hari berubah, memperbarui konten tanpa reload...'
                );
                    ...
                    setTimeout(() => {
                        ...
                        if (prayerLower === 'shubuh') {
                            ...
                            if (adzanShubuhUrl && adzanShubuhUrl
                                ...
                                console.log(
                                    'Memutar audio adzan shubuh'
                                );
                            }
                        } else if (prayerLower !== 'shuruq' &&
                            prayerLower !== 'syuruq' &&
                            prayerLower !== 'terbit') {
        ...
        function showFinalAdzanImage() {
            ...
            const duration = getFinalDuration(currentPrayerName ||
                'Dzuhur');
            ...
            $.ajax({
                ...
                success: async function(response) {
                    ...
                    if (response.success) {
                        ...
                        if (hasChanges) {
                            console.log(
                                'Perubahan terdeteksi, memperbarui slide...'
                            );
            ...
            $.ajax({
                ...
                success: function(response) {
                    console.log('Respons API jumbotron:',
                        response);
                    if (response.success && response.data) {
                        $('#jumbo1').val(response.data
                            .jumbo1 || '');
                        $('#jumbo2').val(response.data
                            .jumbo2 || '');
                        $('#jumbo3').val(response.data
                            .jumbo3 || '');
                        $('#jumbo4').val(response.data
                            .jumbo4 || '');
                        $('#jumbo5').val(response.data
                            .jumbo5 || '');
                        $('#jumbo6').val(response.data
                            .jumbo6 || '');
                        $('#jumbo_is_active').val(response
                            .data.is_active ? 'true' :
                            'false');
                        $(document).trigger(
                            'jumbotronUpdated');
        ...
        function manageSlideDisplay() {
            ...
            async function initSlider() {
                try {
                    ...
                    console.log(
                        'Semua gambar telah dimuat, memulai slider',
                        allUrls);
                    ...
                    function updateSlide() {
                        ...
                        if (isJumbotronActive && imageIndex ===
                            ...
                            @if (config('app.debug') == true)
                                console.log(
                                    `Mosque-image ditampilkan: Index ${slideIndex}, URL ${currentUrl}`
                                );
                            @endif
                        }
                    ...
                    $(document).on('slidesUpdated', async function(
                        event, newSlides) {
                        console.log(
                            'Event slidesUpdated diterima, memperbarui slider'
                        );
                        const newUrls = newSlides
                            .filter(url => url
                                .trim() !== '');
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (response.success) {
                        ...
                        const previousKhatib = $('#khatib')
                            .val();
                        const previousImam = $('#imam')
                            .val();
                        const previousMuadzin = $(
                            '#muadzin').val();

                        // Perbarui nilai input hidden untuk petugas
                        $('#khatib').val(response.data
                            .khatib);
                        $('#imam').val(response.data.imam);
                        $('#muadzin').val(response.data
                            .muadzin);
                ...
                error: function(xhr, status, error, retryCount =
                    0) {
                    console.error(
