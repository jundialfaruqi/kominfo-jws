        ...
        <tbody>
            @foreach ($user as $users)
                <tr>
                    ...
                    <td>
                        {{ $users->profil->name ?? '-' }}
                        @php $slug = $users->profil?->slug; @endphp

                        @if ($slug)
                            @php $url = route('firdaus', $slug); @endphp
                            <a href="{{ $url }}" target="_blank"
                                class="d-flex align-items-center gap-2 text-blue-600 hover:underline">
